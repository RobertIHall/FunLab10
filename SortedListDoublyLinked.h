...
T* SortedListDoublyLinked<T>::remove(DoubleNode<T>* curr)
{
   ...
   //If our previous from our current is null we set After's previous to previous, thus making curr NULL.
   if (prev == NULL)
   {
		after->setPrev(prev);
		loc = after;
   }
   //If out after from our current is null, we set out Previous's after to be after, thus making our curr NULL.
   else if (after == NULL)
   {
		prev->setNext(after);
		loc = prev;
   }
   //If the others fail, we set previous's next to be after and after's previous to previous. Breaking our chain from current.
   else
   {
		prev->setNext(after);
		after->setPrev(prev);
